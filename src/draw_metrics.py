...
def drawMetrics(test_auc_list, test_aupr_list, cell_name, feature_name, model_name):
    # polt
    ...
    plt.xlabel("epoch")
    plt.legend()
    plt.savefig("../model/%s_%s_%s.png" % (cell_name, feature_name, model_name.split()[-1]), dpi=300)
    plt.close()
    # plt.show()
