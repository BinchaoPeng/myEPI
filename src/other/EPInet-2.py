...
embedding_matrix = torch.as_tensor(np.load("../model/embedding_matrix.npy"))
...
if __name__ == '__main__':
    ...
    for epoch in range(1, N_EPOCHS + 1):
        y_pred, y_test, loss = trainModel(epoch)
        print(f"============================[{time_since(start)}]train: EPOCH {epoch} is over!================")
        auc, aupr = score(y_pred, y_test)
        train_auc_list.append(auc)
        train_aupr_list.append(aupr)
        print(f"============================[{time_since(start)}]train score: EPOCH {epoch} is over!================")
        auc, aupr = testModel()
