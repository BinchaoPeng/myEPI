...
from tensorflow.keras.layers import Dense, Activation, Conv1D, ZeroPadding1D, MaxPooling1D, Dropout, Flatten, Conv2D, \
    MaxPooling2D
...
# Plot model history
history = model.fit(np.asarray(X_train).reshape(len(np.asarray(X_train)), num_features, 1),
                    utils.to_categorical(y_train, nb_classes),
                    validation_data=(np.asarray(X_test).reshape(len(np.asarray(X_test)), num_features, 1),
                                     utils.to_categorical(y_test, nb_classes)),
...
# Plot model history
_2D_history = _2D_model.fit(np.asarray(X_train).reshape(len(np.asarray(X_train)), 768, 200, 1),
                            utils.to_categorical(y_train, nb_classes),
                            validation_data=(np.asarray(X_test).reshape(len(np.asarray(X_test)), 768, 200, 1),
                                             utils.to_categorical(y_test, nb_classes)),
                            epochs=nb_epochs, batch_size=16, verbose=1)
